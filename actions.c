...
void remove_copy_files(FileInfo *head) {
        ...
        if (get_original_name_from_copy(curr->name, original_name)) {
            ...
            if (original_file != NULL) {
                if (check_duplicate(curr, original_file)) {
                    ...
                    printf("⚠️[JUMP]: %s 파일명은 복사본 같지만, 원본과 내용이 다릅니다.\n", curr->name);
                }
            }
        }
        curr = curr->next;
    ...
    printf("✅ %d개의 중복 파일 삭제 완료.\n", removed_count);
    write_log("중복 정리: %d개 삭제됨", removed_count);
}
...
void archive_files(FileInfo *head, int days){
    ...
    printf("\n[Process] %d일 동안 접근하지 않은 파일을 선별 중...\n", days);
    ...
    printf(">> 📂 압축 파일명: [ %s ]\n", auto_name);
    write_log("아카이브 실행: %d일 이상 파일 %d개 -> %s", days, count, auto_name);
...
void classify_files_by_extension(FileInfo *head, const char *base_dest_folder) {
    ...
    printf("\n[Process] 파일 확장자를 분석하여 자동 분류를 시작합니다...\n");
